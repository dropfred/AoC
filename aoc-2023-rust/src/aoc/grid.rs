...
impl<T: Copy + PartialEq> Grid<T> {
    ...
    pub fn explore_weighted<F> (&self, start: (usize, usize), filter: F) -> GridExploreWeightIterator<T, F>
    where F: FnMut((usize, usize), (usize, usize), usize) -> Option<usize>
    {
        GridExploreWeightIterator::new(self, start, filter)
    }
...
impl<T: Copy + PartialEq + std::str::FromStr + std::fmt::Debug> Grid<T> {
    pub fn parse(data: &str, sep: &str) -> Option<Self>
    ...
    pub fn load(data: &str, sep: &str) -> Self
    where <T as std::str::FromStr>::Err: std::fmt::Debug {
...
impl<'a, T, F> GridExploreIterator<'a, T, F>
where
    T: Copy + PartialEq,
    F: FnMut((usize, usize), (usize, usize), usize) -> bool {
...
impl<'a, T, F> Iterator for GridExploreIterator<'a, T, F>
where
    T: Copy + PartialEq,
    F: FnMut((usize, usize), (usize, usize), usize) -> bool {
...
pub struct GridExploreWeightIterator<'a, T, F>
where F: FnMut((usize, usize), (usize, usize), usize) -> Option<usize> {
    grid: &'a Grid<T>,
    filter: F,
    positions: BinaryHeap<Reverse<(usize, (usize, usize), (usize, usize))>>,
    visited: Vec<u64>
}

impl<'a, T, F> GridExploreWeightIterator<'a, T, F>
where
    T: Copy + PartialEq,
    F: FnMut((usize, usize), (usize, usize), usize) -> Option<usize> {
    pub fn new(grid: &'a Grid<T>, start: (usize, usize), filter: F) -> Self {
        let (w, h) = grid.size();
        let positions = BinaryHeap::new();
        let visited = vec![0; h * (w + 63) / 64];
        let mut it = GridExploreWeightIterator {grid, filter, positions, visited};
        it.visit(start, start, 0);
        it
    }
    ...
    fn visit(&mut self, position: (usize, usize), pposition: (usize, usize), distance: usize) {
        ...
        if (self.visited[v] & bx) == 0 {
            self.visited[v] |= bx;
            if let Some(distance) = (self.filter)(position, pposition, distance) {
                self.positions.push(Reverse((distance, position, pposition)));
            }
        }
    }
}

impl<'a, T, F> Iterator for GridExploreWeightIterator<'a, T, F>
where
    T: Copy + PartialEq,
    F: FnMut((usize, usize), (usize, usize), usize) -> Option<usize> {
    type Item = ((usize, usize), (usize, usize), usize);
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(Reverse((distance, position, pposition))) = self.positions.pop() {
            let (w, h) = {let (w, h) = self.grid.size(); (w as i32, h as i32)};
            let (x, y) = (position.0 as i32, position.1 as i32);
            for d in Dir::all() {
                let (dx, dy) = d.offset();
                ...
                if (x >= 0) && (x < w) && (y >= 0) && (y < h) {
                    let p = (x as usize, y as usize);
                    self.visit(p, position, distance);
                }
            }
            Some((position, pposition, distance))
        } else {
...
where
    T: Copy + PartialEq + FromStr + std::fmt::Debug,
